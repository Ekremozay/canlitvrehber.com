...
function shouldUseDirectPlaybackUrl(inputUrl) {
...
function normalizePlaybackUrl(inputUrl) {
  if (!inputUrl) return inputUrl;
  if (!shouldUseDirectPlaybackUrl(inputUrl)) return inputUrl;
  return resolveRealUrl(inputUrl) || inputUrl;
}
...
export function getChannelStreamOptions(channel) {
  ...
  if (Array.isArray(channel.streamOptions) && channel.streamOptions.length > 0) {
    return channel.streamOptions
      ...
      .map((item) => ({
        ...item,
        url: normalizePlaybackUrl(item.url),
      }));
  }
  ...
  if (channel.stream) {
    generated.push({
      ...
      label: "Ana Kaynak",
      url: normalizePlaybackUrl(channel.stream),
    });
  }
