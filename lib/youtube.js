...
export function extractYouTubeChannelId(rawUrl) {
  if (!rawUrl) return "";

  try {
    const url = new URL(rawUrl);
    const channelParam = url.searchParams.get("channel") || "";
    if (/^UC[A-Za-z0-9_-]{22}$/.test(channelParam)) {
      return channelParam;
    }
...
export function buildYouTubeChannelLiveEmbedUrl(channelId) {
  if (!/^UC[A-Za-z0-9_-]{22}$/.test(String(channelId || ""))) return "";
  const params = new URLSearchParams({
    channel: channelId,
    autoplay: "1",
    mute: "1",
...
function detectEmbeddingDisabled(html) {
  return (
    /"playableInEmbed":false/.test(html) ||
    /Playback on other websites has been disabled by the video owner/i.test(html) ||
    /Video unavailable[\s\S]{0,200}watch on YouTube/i.test(html)
  );
}
...
async function fetchYoutubeLiveStatus(rawUrl) {
  ...
  try {
    ...
    const channelId =
      ...
      extractYouTubeChannelId(resolvedUrl) ||
      extractYouTubeChannelId(rawUrl);
    const liveDetected = Boolean(liveVideoId) && looksLikeLiveHtml(html);
    const embeddingDisabled = detectEmbeddingDisabled(html);
    ...
    return {
      ...
      channelId,
      videoId: liveVideoId,
      embedUrl: buildYouTubeChannelLiveEmbedUrl(channelId) || buildYouTubeEmbedUrl(liveVideoId),
      liveUrl: buildYouTubeLiveUrl(liveVideoId),
      watchUrl: `https://www.youtube.com/watch?v=${liveVideoId}`,
      title,
      checkedAt: new Date().toISOString(),
